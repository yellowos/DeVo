...
% pulses_N = pulses sent to EMSP in Newton
...
%% Simulation of EMPS
disp(' ')
disp('Simulation of EMSP with DATA_EMPS_PULSES.mat')

tic; 
options=simset('Solver','ode45','RelTol','auto','AbsTol','auto',...
  'Refine',1,'MaxStep',tec/10,'ZeroCross','off');
sim('Simulink_EMPS_Rigide_Pulse',tf,options);
tps = toc;
h = floor(tps/3600);
...
%% Display results
disp(' ')
disp('Relative errors with EMPS_DATA_PULSES.mat : ')
