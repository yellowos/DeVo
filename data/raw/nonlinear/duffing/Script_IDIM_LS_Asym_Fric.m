...
% Asymmetrical friction accounted for
O_plus  = (1 + sign(dq_f)) /2;
O_minus = (1 - sign(dq_f)) /2;
% Raw observation matrix
X_0 = [ddq_f dq_f.*O_plus sign(dq_f).*O_plus ...
    dq_f.*O_minus sign(dq_f).*O_minus];
% Names of parameters
params = ['M1  ';'FV1p';'FC1p';'FV1m';'FC1m'];
